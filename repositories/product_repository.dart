...
class ProductRepository {
  ...
  Future<ProductMiniResponse> getCategories() async {
  ...
  Future<ProductMiniResponse> getCategoryProducts(
      {@required int id = 0, name = "", page = 1, String ?catego}) async {
    Uri url;
    if(catego==null){
       ...
       url = Uri.parse("${AppConfig.BASE_URL}/products/category/" +
          id.toString() +
          "?page=${page}&name=${name}&selected_attribute_values=${catego}");
    }

print('hgfg'+url.toString());
    final response = await http.get(url, headers: {
      "App-Language": app_language.$,
    });
    return productMiniResponseFromJson(response.body);
...
var post_body = jsonEncode({
  ...
  Future<ProductDetailsResponse> getProductDetailslogin(
      {@required int id = 0}) async {

    Uri url = Uri.parse("${AppConfig.BASE_URL}/user/products/" + id.toString());
    print(url.toString());
    final response = await http.get(url, headers: {
      "App-Language": app_language.$,
      "Authorization": "Bearer ${access_token.$}",
