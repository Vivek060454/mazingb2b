...
class _CartState extends State<Cart> {
          ...
          shop.cartItems.forEach((cartItems) {
            _cartTotal += double.parse(
                ((cartItems.price + cartItems.tax) * cartItems.quantity)
                    .toStringAsFixed(2));
            _cartTotalString =
                "${cartItems.currencySymbol}${_cartTotal.toStringAsFixed(2)}";
          });
        }
      });
    }
  ...
  partialTotalString(index) {
    ...
    if (_shopList[index].cartItems.length > 0) {
      _shopList[index].cartItems.forEach((cartItems) {
        ...
        print('###############'+partialTotal.toString());
        print('########price#######'+cartItems.price.toString());
        print('########tax#######'+cartItems.tax.toString());
        print('########quantity#######'+cartItems.quantity.toString());

        partialTotalString =
            "${cartItems.currencySymbol}${partialTotal.toStringAsFixed(2)}";
      });
    }
  ...
  onQuantityIncrease(seller_index, item_index) {
    if (_shopList[seller_index].cartItems[item_index].quantity <
      ...
      ToastComponent.showDialog(
          "${'Cannot order more than one'
              // AppLocalizations.of(context)!.cart_screen_cannot_order_more_than
          } ${_shopList[seller_index].cartItems[item_index].upperLimit} ${
          'Cart Screen Item For this'
              // AppLocalizations.of(context)!.cart_screen_items_of_this
          }",
    ...
    if (_shopList[seller_index].cartItems[item_index].quantity >
      ...
      ToastComponent.showDialog(
          "${'cannot add more than one item'
              // AppLocalizations.of(context)!.cart_screen_cannot_order_more_than
          } ${_shopList[seller_index].cartItems[item_index].lowerLimit} ${'cart for this screen'
              // AppLocalizations.of(context)!.cart_screen_items_of_this
          }",
          gravity: Toast.center,
          duration: Toast.lengthLong);
    ...
    showDialog(
        ...
        builder: (_) => AlertDialog(
              ...
              content: Padding(
                ...
                child: Text('Are you sure to remove this item',
  ...
  process({mode}) async {
    ...
    if (cart_ids.length == 0) {
      ToastComponent.showDialog('Cart empty',
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _shopList[seller_index]
                        .cartItems[item_index]
                        .piecePerCarton==0? Padding(
                      padding: const EdgeInsets.only(top: 8),
                      child: Text('Order by piece',
                        overflow: TextOverflow.ellipsis,
                        maxLines: 2,
                        style: TextStyle(
                            color: Colors.black,
