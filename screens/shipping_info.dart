...
class _ShippingInfoState extends State<ShippingInfo> {
  ...
  final TextEditingController shipername = new TextEditingController();
  final TextEditingController gstin = new TextEditingController();
  ...
  fetchAll() {
    getDeliveryInfo();
    print('efgrgbteh#######${carrier_base_shipping.$}');
  }
...
/*
  ...
  final storage1 = new LocalStorage('my_data1');
  ...
  changeShippingOption(ShippingOption option, index) {
    ...
    if (option.index == 2) {
      if(_deliveryInfoList.first.carriers!.isNotEmpty)
      _sellerWiseShippingOption[index].shippingId =
          _deliveryInfoList.first.carriers!.first.id!;
    }
  ...
  onPressProceed(context) async {
    ...
    if(_sellerWiseShippingOptionValidation.isNotEmpty &&  carrier_base_shipping.$){
      ToastComponent.showDialog('Invalid',
          // LangText(context).local.common_invalid_warning,
   ...
   var shipper="id";
    ...
    await AddressRepository().getShippingCostResponse(
        // shipping_type: _sellerWiseShippingOption
        storage1.getItem('id').toString(),
      shipername.text,
      gstin.text,
      phone.text,
        storage.getItem('cari')
    ...
    if (shippingCostResponse.result == false) {
      ToastComponent.showDialog('Network error',
          // LangText(context).local.common_network_error,
          gravity: Toast.center, duration: Toast.lengthLong);
      return;
    }
  ...
  RefreshIndicator buildBody(BuildContext context) {
   ...
   buildBodyChildren(BuildContext context) {
    ...
    return AppBar(
      ...
      title: Text(
        "${'Shipping cost'
            // AppLocalizations
            // .of(context)
            // !.shipping_info_screen_shipping_cost
        } ${_shipping_cost_string}",
...
*/
  ...
  Widget buildCarrierSection(sellerArrayIndex) {
    if (is_logged_in.$ == false) {
      ...
      return buildCarrierShimmer();
    } else if (_deliveryInfoList[sellerArrayIndex].carriers!.length > 0) {
      return Container(
  ...
  Widget buildCarrierListView(sellerArrayIndex) {
    return ListView.separated(
      itemCount: _deliveryInfoList[sellerArrayIndex].carriers!.length,
      scrollDirection: Axis.vertical,
  ...
  GestureDetector buildCarrierItemCard(carrierIndex, sellerArrayIndex) {
    return GestureDetector(
      onTap: () {
        if (_sellerWiseShippingOption[sellerArrayIndex].shippingId !=
            _deliveryInfoList[sellerArrayIndex].carriers![carrierIndex]
                .id) {
          _sellerWiseShippingOption[sellerArrayIndex].shippingId =
              _deliveryInfoList[sellerArrayIndex].carriers![carrierIndex]
                  .id!;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        _deliveryInfoList[sellerArrayIndex].carriers
                            ![carrierIndex].transitTime.toString() + " " +'days',
        ...
        child: Row(
          ...
          children: [
            Btn.minWidthFixHeight(
              ...
              onPressed: () {
                if(storage.getItem('cari')==null){
                  ToastComponent.showDialog(
                      'Please select carriers',
                      gravity: Toast.center,
                      duration: Toast.lengthLong);
                }
                else{
                  if(storage.getItem('cari')=="Other"){
    ...
    return Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
'Choose delivery',
                  // LangText(context).local.shipping_info_screen_address_choose_delivery,
                  style: TextStyle(
                      color: MyTheme.dark_font_grey,
                      fontWeight: FontWeight.w700,
                      fontSize: 12),
              ...
              SizedBox(height: 5,),
              // Text(  _deliveryInfoList[index].carriers[0].name.toString()),
              Builder(
                  builder: (context) {
                     // final List<dynamic>varii =  _deliveryInfoList[index].carriers;
                     // var variantee=  _deliveryInfoList[index].carriers==null?null:varii[0];
                    var variantee;
                    return Table(
                      columnWidths: {
                    ...
                    height: 50,
                      child: Padding(
                        padding: const EdgeInsets.only(right: 0,bottom: 0,left: 8),
                        child: Container(
                          // width: 80,
                          decoration: BoxDecoration(
                            color: Colors.white,
                          ...
                          child: Padding(
                            ...
                            child:DropdownButtonHideUnderline(
                              child: DropdownButton(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Text(storage.getItem('cari')==null?'Select carrier':storage.getItem('cari').toString(),maxLines: 1,overflow: TextOverflow.ellipsis,
                                  ...
                                  value: variantee,
                                  items:   _deliveryInfoList[index].carriers
                                      !.map((ite) => DropdownMenuItem(
                                    value: ite,
                                    child: Padding(
                                      padding:  EdgeInsets.only(top: 0,bottom: 0,right: 13,left: 10),
                                      ...
                                      .toList(),
                                  onChanged: (dynamic items) {

                                    setState(() => variantee = items.name.toString());
                                    storage.setItem('cari', variantee);
                                    storage1.setItem('id', items.id);
                                    variantee = items.name;
                                    print(variantee+'rgs');
                                    // Navigator.push(context, MaterialPageRoute(builder: (context) {
                            ...
                            InkWell(
                              ...
                              onTap:(){
                                ...
                                print(variantee);
                                storage.setItem('cari', variantee);
                                storage1.setItem('id', 372);
                                 setState(() => variantee = 'Other');
                                // setState(() {
                                //
                                // });
                              },
                              child: Container(
                                ...
                                child: Center(
                                  child: Padding(
                                    ...
                                    child: Text('Other',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w400,color: Colors.white),),
                                  ),
                                ),
                              ),
                            )
                    ...
                    child: Column(
                      children: [
                            ...
                            Padding(
                              ...
                              child: TextFormField(
                                autofocus: false,
                                controller: shipername,
                                keyboardType: TextInputType.name,
                                onSaved: (value) {
                                  shipername.text = value!;
                                },
                                ...
                                validator: (shipername) {
                                  if (shipername!.isEmpty) {
