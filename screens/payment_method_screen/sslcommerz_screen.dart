...
class _SslCommerzScreenState extends State<SslCommerzScreen> {
  ...
  getSetInitialUrl() async {
    ...
    if (sslcommerzUrlResponse.result == false) {
      ToastComponent.showDialog(sslcommerzUrlResponse.message.toString(), gravity: Toast.center, duration: Toast.lengthLong);
      Navigator.of(context).pop();
      return;
    }
  ...
  void getData() {
    _webViewController
      ...
      Map<String, dynamic> responseJSON = jsonDecode(data as String);
      //print(data.toString());
      ...
      return Container(
        child: Center(
          child: Text('Creating Order'
              // AppLocalizations.of(context)!.common_creating_order
          ),
        ),
      );
      ...
      return Container(
        child: Center(
          child: Text('Fetching url'
              // AppLocalizations.of(context)!.sslcommerz_screen_fetching_sslcommerz_url
          ),
        ),
      );
    } else {
...
backgroundColor: Colors.white,
      ...
      title: Text('Pay with sslcommerz',
